...
public class LikeServiceImpl implements LikeService {
	...
	public void addLike(int bno, String id) throws Exception {
		int liker_id = memberDao.selectById(id).getUser_no();
		LikeVO vo = new LikeVO(bno, liker_id);
		if(likeDao.isLike(vo) != 1) { 
			likeDao.addLike(vo);
			if(boardDao.boardDetail(bno).getWriter_id() != liker_id) { //작성자와 좋아요 누른 사람이 일치하지 않으면 -> 좋아요 알림
				NoticeVO notice = new NoticeVO(liker_id, boardDao.boardDetail(vo.getBno()).getWriter_id(), "like");
	...
	public void deleteLike(int bno, String id) throws Exception {
		int like_id = memberDao.selectById(id).getUser_no();
